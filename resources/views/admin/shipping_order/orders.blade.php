                        ...
                        <table class="table table-striped">
                            ...
                            @foreach($orders as $order)
                                <tr>
                                    ...
                                    <td>{{$order['date']}}</td>
                                    <td>{{$order['price']}}</td>
                                    <td>{{$order['from_city']->text}}</td>
                                    <td><a href="{{route('Client',$order['client_id'])}}">Профиль</a></td>
                                    @if ($order['step'] > 1)
                                        <td><a href="{{route('Driver',$order['driver_id'])}}">Профиль</a></td>
                                    @else
                                        <td></td>
                                    @endif
                                    <td>
                                    ...
                                    <td>
                                        <a  href="{{route('ShippingOrderDelete',$order['id'])}}"  class="btn btn-danger waves-effect">удалить </a>
                                    </td>
                                </tr>
                            @endforeach
